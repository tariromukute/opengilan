...
def print_json_stats():
    if args.latency:
        print_json_latency_stats()
    else:
        print_json_count_stats()
...
def print_json_count_stats():
    data = bpf["data"]
    for k, v in sorted(data.items(), key=lambda kv: -kv[1].value)[:args.top]:
        if k.value == 0xFFFFFFFF:
            continue    # happens occasionally, we don't need it
        obj = { agg_colname: agg_colval(k).decode("utf-8") , 'count': v.value }
    print("")
    data.clear()

def print_json_latency_stats():
    data = bpf["data"]
    for k, v in sorted(data.items(),
                       key=lambda kv: -kv[1].total_ns)[:args.top]:
        if k.value == 0xFFFFFFFF:
            continue    # happens occasionally, we don't need it
        obj = { agg_colname: agg_colval(k).decode("utf-8"), "count": v.count, time_colname: v.total_ns / (1e6 if args.milliseconds else 1e3)  }
        print(obj)
    data.clear()
