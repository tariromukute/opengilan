...
def print_json_event(cpu, data, size):
    event = b["events"].event(data)
    obj = { "TIME": strftime("%H:%M:%S"), "PID": event.pid, "COMM":  event.comm.decode('utf-8', 'replace'), "AGE(s)": float(event.delta) / 1000, "FILE":  event.fname.decode('utf-8', 'replace') }
    print(obj)

b["events"].open_perf_buffer(print_json_event)
while 1:
    try:
        b.perf_buffer_poll()
