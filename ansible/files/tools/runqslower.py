...
def print_json_event(cpu, data, size):
    event = b["events"].event(data)
    if args.previous:
        obj = { "time": strftime("%H:%M:%S"), "comm": event.task, "tid":  event.pid, "lat(us)": event.delta_us, "prev comm": event.prev_task, "prev tid": event.prev_pid }
        print(obj)
    else:
        obj = { "time": strftime("%H:%M:%S"), "comm": event.task, "tid": event.pid, "lat(us)": event.delta_us }
        print(obj)
# read events
# load BPF program
b = BPF(text=bpf_text)
...
# read events
b["events"].open_perf_buffer(print_json_event, page_cnt=64)
while 1:
    try:
        b.perf_buffer_poll()
