...
def _print_json_event(cpu, data, size): # callback
    """Print the exit event."""
    e = ct.cast(data, ct.POINTER(Data)).contents
    if Global.args.timestamp:
        now = datetime.utcnow() if Global.args.utc else datetime.now()
    ...
    # PCOMM            PID    PPID   TID    AGE(s)  EXIT_CODE
    age = (e.exit_time - e.start_time) / 1e9
    obj = { "pcomm": e.task.decode(), "pid": e.pid, "ppid": e.ppid, "tid": e.tid, "age": age }
    if e.sig_info == 0:
        # print("0" if e.exit_code == 0 else "code %d" % e.exit_code)
        obj["code"] = "0" if e.exit_code == 0 else "code %d" % e.exit_code
    else:
        sig = e.sig_info & 0x7F
        if sig:
            # print("signal %d (%s)" % (sig, signum_to_signame(sig)), end="")
            obj["code"] = "signal %d (%s)" % (sig, signum_to_signame(sig))
        if e.sig_info & 0x80:
            # print(", core dumped ", end="")
            obj["code"] = "core dumped"
        # print()
    print(obj)
...
def main():
    try:
        ...
        snoop(buffer, _print_json_event)
